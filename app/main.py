...
class Service:
    def __init__(self) -> None:
        ...
        self.flight = FlightTracker.from_dict(persisted)
        # last KNOWN aircraft fix, kept across offline so the map can show where it went dark
        self.last_ac = persisted.get("last_ac")
        self.last_seen_ts = persisted.get("last_seen_ts")
        ...
        self.snapshot = {
            ...
            "last_ac": self.last_ac, "last_seen_ts": self.last_seen_ts,
            "airborne_since": self.flight.airborne_since, "dist_km": round(self.flight.dist_km, 1),
    ...
    async def poll_once(self, session: aiohttp.ClientSession) -> None:
        ...
        self.flight.update(now, new_presence, lat, lon)
        if ac and lat is not None:  # remember the last good fix across future offline periods
            self.last_ac = ac
